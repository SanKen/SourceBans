...
class ETwigViewRenderer extends CApplicationComponent implements IViewRenderer
{
    ...
    function init()
    {
        ...
        // Adding core static classes proxy as 'C' and 'SB' shortcut (usage: {{C.Html.tag(...)}})
        $this->_twig->addGlobal('C', new ETwigViewRendererCoreStaticClassesProxy('C'));
        $this->_twig->addGlobal('SB', new ETwigViewRendererCoreStaticClassesProxy('SB'));
...
/**
 * Class-proxy for core static classes
 *
 * @author Leonid Svyatov <leonid@svyatov.ru>
 * @version 1.0.0
 */
class ETwigViewRendererCoreStaticClassesProxy
{
    ...
    {
        return (isset($_classes[$className]) || class_exists($this->_prefix.$className));
    }

    function __get($className)
    {
        if (!isset($this->_classes[$className])) {
            $this->_classes[$className] = new ETwigViewRendererStaticClassProxy($this->_prefix.$className);
        }
